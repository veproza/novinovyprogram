...
const ftParser: IParser = async (file) => {
    const content = await downloadObject(file);
    const root = parse(content.toString());

    const elements = ((root as any).querySelectorAll('.o-teaser'));
    return elements
        .map((e: any): IArticleData | null => {

            const headlineElement = e.querySelector('.o-teaser__heading');
            if(!headlineElement) {
                return null;
            }
            ...
            const link =  href.startsWith('/') ? 'https://www.ft.com' + href : href;
